// MARK: DISCLAIMER
// There is no Process View like in the other camera process (Topography)
// because the kernel shader here is more strict and standardaized so unexpected results are uncommon,
// Allowing for a more strict algorithm to determine capture.
// Because of this there is no need for the user to determine if the capture is good because
// the variables are mostly static and unchanging compared
// to live data where there is more movement.
...
internal class CalibrationCamera: MTKViewController {
...
extension CalibrationCamera: MetalCameraSessionDelegate {
...
// MARK: Calibration Funcs
//  release the image once the data points are gained,
// overide when new calibration is gathered.
// Data smoothing,
// Zernike graph print out for future computation?

// MARK: Navigation
// Instead of ProcessView Set up loading and have save or retry
